...
- **No prompt/rule content in code.** All lyric and generation rule TEXT — lyric draft/edit prompts, ban entries, outcome lyric factors, genre craft overlays, Mars contamination terms, Mars axis rules — lives in the DB and is editable in Dash → Prompts & Rules. Code holds schemas, loaders, and structural formatters; code does NOT hold rule strings. Bump/upgrade seed scripts under `prisma/seed/bump-bernie-*` and `prisma/seed/upgrade-lyric-factors.ts` are historical artifacts (header-marked DEPRECATED on 2026-05-25). For any new prompt or rule change, edit the DB row through Dash — don't write a new bump script. Migrating a rule surface into code is a regression; flag it on PR review.
