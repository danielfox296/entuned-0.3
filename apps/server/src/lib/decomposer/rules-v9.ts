...
// MusicologicalRules v9 — redundancy trim on top of v8.
//   v8 → v9 changes:
//     - DROP per-field word-count restatements ("≤25 words" / "≤30 words" /
//       "≤20 words" trailing on each field guidance). The global rule
//       "Comma-fragment style, ~25 words per field" earlier in the prompt
//       already covers this; restating per-field was redundant token spend.
//     - TRIM the GOOD-example arrangement_sections JSON from five sections
//       (intro/verse/chorus/bridge/outro) to two (verse, chorus). One
//       example shows vocal_delivery, the other shows dynamic — enough to
//       teach the shape without padding.
//   Output schema is unchanged from v8 — still 11 keys.

export const MUSICOLOGICAL_RULES_V9 = `
# Musicological Rules — v9
