...
| `mars/` | Style-prompt builder for Suno. Anchor-and-carve: genre anchor + negative-style axes. Every output is wrapped by `applyOutcomeFactorPrompt` from `eno`. | `marsAssemble` ([mars/mars.ts](mars/mars.ts)); anchor strategy `buildAnchorStyle` ([mars/style-anchor.ts](mars/style-anchor.ts)) |
