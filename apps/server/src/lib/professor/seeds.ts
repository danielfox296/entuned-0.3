...
LLM failure: Defaults to grammatically complete declarative sentences. Pads short emotional cores with explanatory clauses ("...when I lean back", "...so I knew it was time", "...like I always do"). Treats the hook the same way it treats a verse — same word count, same syntactic closure — losing the breath that makes a hook stick. Compounds the problem by deferring to Module 9 (Intentional violations) at every hook, reasoning that the hook's length is "the writer's choice" — which defaults to no action.
...
Override clause: This module supersedes Module 9 at landing positions. Compression IS the intentional choice at the hook and at section closes — apply it unless the longer form is performing a SPECIFIC named effect (pointed redundancy, established voice idiosyncrasy from earlier in the song, deliberate hook-irony that prior chorus iterations earned). If you cannot name the specific work the trailing clause is doing, default to compression. "It feels intentional" is not a named effect; "the writer probably meant it" is not a named effect.`,
  },
  {
