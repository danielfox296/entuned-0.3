...
//   GET    /admin/style-prompt                   — { latest, history } for the anchor builder
//   POST   /admin/style-prompt                   — new style (anchor) prompt version
...
const GenreGravityRulePostBody = z.object({
  ...
  vocalCharacters: z.array(z.string().min(1)).default([]),
  vocalDeliveries: z.array(z.string().min(1)).default([]),
  vocalEffects: z.array(z.string().min(1)).default([]),
...
const GenreGravityRulePatchBody = z.object({
  ...
  vocalCharacters: z.array(z.string().min(1)).optional(),
  vocalDeliveries: z.array(z.string().min(1)).optional(),
  vocalEffects: z.array(z.string().min(1)).optional(),
  notes: z.string().nullable().optional(),
  active: z.boolean().optional(),
})
...
export const adminRoutes: FastifyPluginAsync = async (app) => {
    ...
    try {
      const row = await prisma.genreGravityRule.create({
        data: {
          ...
          vocalCharacters: parsed.data.vocalCharacters,
          vocalDeliveries: parsed.data.vocalDeliveries,
          vocalEffects: parsed.data.vocalEffects,
          notes: parsed.data.notes ?? null,
  ...
  // DB-backed system prompt for the anchor-and-carve style builder. Same shape
  // as /lyric-prompts. Cold-start v1 happens in lib/mars/style-anchor.ts on
  // first call. Schema SSOT: ../../../entune v0.3/schema/light-cards.md
  ...
  const OutcomeCreateBody = z.object({
    ...
    // Symmetric BPM spread band — each seed samples a concrete tempo in [tempoBpm ± radius].
    tempoBpmRadius: z.number().int().min(0).max(60).nullable().optional(),
    mode: z.string().min(1),
    // Weighted mode distribution, e.g. { "major": 0.7, "minor": 0.3 } — each seed samples one.
    modeWeights: z.record(z.string().min(1), z.number().positive()).nullable().optional(),
    mood: z.string().min(1),
  ...
  app.post('/outcomes', async (req, reply) => {
    ...
    const row = await prisma.outcome.create({
      data: {
        ...
        tempoBpmRadius: parsed.data.tempoBpmRadius ?? null,
        mode: parsed.data.mode,
        modeWeights: parsed.data.modeWeights ?? Prisma.JsonNull,
