...
- **The Client → Location → ICP selector is store-OPTIONAL.** It lives in `src/ui/SelectionCascade.tsx` and both the Clients and Workflows shells use it. Picking "— all locations —" lists the client's own ICPs; picking a location lists that location's ICPs. Panels scoped to an ICP (ICP Editor, Reference Tracks, Hook Writing, Pipeline) must read `api.icpDetail(icpId)` and gate on `icpId` — **never** on `storeId`, and never source ICPs from `api.storeDetail`. A Station's ICP has no `StoreICP` link until a store picks that station, so a `storeId` gate makes all six launch stations unreachable and there is no way to give them reference tracks. Only genuinely location-scoped panels (Launch Checklist, Location, Campaigns, Schedule) gate on `storeId`.
- **A location's ICPs are not always its client's ICPs.** Every free location links to the Free Tier client's pools. Fetch a location's ICPs with `api.icps(token, { storeId })`, not `{ clientId }`.
