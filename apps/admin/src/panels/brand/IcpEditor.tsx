...
function ReferenceTracks({ detail: _detail, icp, onChanged }: { detail: StoreDetail; icp: IcpWithRefs; onChanged: () => void }) {
  ...
  const [bulkBusy, setBulkBusy] = useState<null | { kind: 'approve' | 'decompose'; done: number; total: number }>(null)
  const toast = useToast()
  ...
  const pending = icp.referenceTracks.filter((r) => r.status === 'pending')
  const undecomposedApproved = icp.referenceTracks.filter((r) => r.status !== 'pending' && r.styleAnalysis === null)
  ...
  // Bulk operations: serialized, not parallel — the per-track endpoints are
  // backed by Claude calls and concurrent bursts silently rate-limit (B3).
  const approveAllPending = async () => {
    const token = getToken(); if (!token) return
    if (pending.length === 0) return
    setBulkBusy({ kind: 'approve', done: 0, total: pending.length })
    let ok = 0; let failed = 0
    for (let i = 0; i < pending.length; i++) {
      try { await api.approveReferenceTrack(pending[i].id, token); ok++ }
      catch { failed++ }
  ...
  const decomposeAllUndecomposed = async () => {
    const token = getToken(); if (!token) return
    if (undecomposedApproved.length === 0) return
    setBulkBusy({ kind: 'decompose', done: 0, total: undecomposedApproved.length })
    let ok = 0; let failed = 0
    for (let i = 0; i < undecomposedApproved.length; i++) {
      try { await api.decomposeReferenceTrack(undecomposedApproved[i].id, false, token); ok++ }
      catch { failed++ }
      ...
      <div style={{ marginBottom: 12, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        {pendingCount > 0 && (
          <button
            ...
            disabled={!!bulkBusy}
            style={primaryBtn(!bulkBusy, bulkBusy?.kind === 'approve')}
            title={`Approve all ${pendingCount} pending suggestions`}
          >{bulkBusy?.kind === 'approve'
              ? `approving ${bulkBusy.done}/${bulkBusy.total}…`
              : `approve all (${pendingCount})`}</button>
        )}
        {undecomposedApproved.length > 0 && (
          <button
            onClick={decomposeAllUndecomposed}
            disabled={!!bulkBusy}
            style={primaryBtn(!bulkBusy, bulkBusy?.kind === 'decompose')}
            title={`Decompose all ${undecomposedApproved.length} undecomposed approved tracks`}
          >{bulkBusy?.kind === 'decompose'
              ? `decomposing ${bulkBusy.done}/${bulkBusy.total}…`
              : `decompose all (${undecomposedApproved.length})`}</button>
        )}
