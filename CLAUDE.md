...
- **Railway's build runs server tests with `apps/server` mounted at `/app` — the monorepo root is NOT at a fixed relative path.** Tests that read root files (`pnpm-lock.yaml`, root `package.json`) must walk up to find them and skip gracefully when absent (bit the dependency-floors deploy, 2026-08-10; see `apps/server/src/lib/dependencyFloors.test.ts`).
- **Pushing to `main` auto-deploys the Railway service** — a manual `railway up` on top of a push creates a second, racing deployment. Push, then watch the auto-deploy; reserve `railway up` for deploying uncommitted work.
